...
def Errorpropagation(f, par, con = None, rho = None, cov = None):
    ...
    if rho is not None:
        rho_xy = [[1,rho],[rho,1]] # correlation matrix
        for i in range(len(par)):
            for j in range(len(par)):
                correlation_error += rho_xy[i][j] * error_par[i] * error_par[j] * sp.diff(f, par[i]) * sp.diff(f, par[j])
...
def easy_hist(x, Nbins, Figsize=(10, 7), xrange=None, title= None, x_label= 'x'):
    ...
    ax.set(xlabel=x_label, ylabel=f'Counts / {binwidth:.2}', title=title)
    # plt.show()
